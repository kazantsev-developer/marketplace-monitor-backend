...
func LoadConfig() (*Config, error) {
	...
	// Загружаем секции с префиксами
	if err := envconfig.Process("WB", &cfg.WB); err != nil {
		return nil, fmt.Errorf("WB: %w", err)
	}
	if err := envconfig.Process("DB", &cfg.DB); err != nil {
		return nil, fmt.Errorf("DB: %w", err)
	}
	if err := envconfig.Process("OZON", &cfg.Ozon); err != nil {
		return nil, fmt.Errorf("OZON: %w", err)
	}
	if err := envconfig.Process("MS", &cfg.MS); err != nil {
		return nil, fmt.Errorf("MS: %w", err)
	}
	if err := envconfig.Process("", &cfg.Settings); err != nil {
		return nil, fmt.Errorf("SETTINGS: %w", err)
	}

	// Загружаем корневые поля (Port, Env) без префикса
